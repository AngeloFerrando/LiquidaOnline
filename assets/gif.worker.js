...
(function e(t, n, r) {
  ...
  1: [function (require, module, exports) {
    ...
    ByteArray.charMap = {};
    for (i = 0; i < 256; i++) ByteArray.charMap[i] = String.fromCharCode(i);
    ...
    ByteArray.prototype.writeUTFBytes = function (string) {
      ...
      let i = 0;
      for (; i < l; i++) this.writeByte(string.charCodeAt(i))
    };
    ByteArray.prototype.writeBytes = function (array, offset, length) {
      const l = length || array.length;
      let i = offset || 0;
      for (; i < l; i++) this.writeByte(array[i])
    };
    ...
    GIFEncoder.prototype.ditherPixels = function (kernel, serpentine) {
      ...
      let index = 0;
      const height = this.height, width = this.width, data = this.pixels;
      let direction = serpentine ? -1 : 1;
      this.indexedPixels = new Uint8Array(this.pixels.length / 3);
      for (let y = 0; y < height; y++) {
        if (serpentine) direction = direction * -1;
        let x = direction === 1 ? 0 : width - 1;
        const xend = direction === 1 ? width : 0;
        for (; x !== xend; x += direction) {
          index = y * width + x;
          let idx = index * 3;
          const r1 = data[idx];
          const g1 = data[idx + 1];
          ...
          const end = direction === 1 ? ds.length : 0;
    ...
    function NeuQuant(pixels, samplefac) {
      ...
      function inxbuild() {
        let i, j, p, q, smallpos, smallval, previouscol = 0, startpos = 0;
        for (i = 0; i < netsize; i++) {
          p = network[i];
          smallpos = i;
          smallval = p[1];
